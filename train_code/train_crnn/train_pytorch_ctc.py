...
config.saved_model_prefix = 'CRNN-0305'  # 模型名称
config.train_infofile = 'D:/Code/Python/29invoice/ocr/train_code/train_crnn/train_data'  # 数据集路径
config.val_infofile = 'D:/Code/Python/29invoice/ocr/train_code/train_crnn/train_data'
...
def val(net, dataset, criterion, max_iter=100):
    ...
    if config.use_log:
        with open(log_filename, 'a') as f:
            f.write('ocr_acc:{}/n'.format(accuracy))
    global best_acc
...
for epoch in range(config.niter):
    ...
    if config.use_log:
        with open(log_filename, 'a') as f:
            f.write('{}/n'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')))
            f.write('train_loss:{}/n'.format(loss_avg.val()))
