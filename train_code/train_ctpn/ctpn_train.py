...
def save_checkpoint(state, epoch, loss_cls, loss_regr, loss, ext='pth'):  # 保存模型
    check_path = os.path.join(config.checkpoints_dir,
                              f'ctpn_ep{epoch:02d}_'  # 模型名称
...
if __name__ == '__main__':
    ...
    # 实例化dataset，训练的时候会往里面去读
    dataset = VOCDataset(config.img_dir, config.xml_dir)
    ...
    for epoch in range(resume_epoch + 1, epochs):
        ...
        # if epoch % 30 == 0 and epoch != 0:  # 保存模型
        if epoch != 0:
            torch.save(model.state_dict(), os.path.join(config.checkpoints_dir, 'ctpn_' + str(epoch) + '.pth'))
